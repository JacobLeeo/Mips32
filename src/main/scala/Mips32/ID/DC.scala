...
class DC extends Module {
    ...
    op := io.inFromD.iRRdDt(31, 26) // 指令操作码
    io.outToD.rWAddr := io.inFromD.iRRdDt(20, 16) // 寄存器写地址
    io.outToRF.r1RAddr := io.inFromD.iRRdDt(25, 21) // 寄存器1读地址
    io.outToRF.r2RAddr := io.inFromD.iRRdDt(20, 16) // 寄存器2读地址
    ...
    switch(op) {
        is ("b001101".asUInt) { // 指令码为001101
            ...
            imm := io.inFromD.iRRdDt(15, 0) // 获取指令中的立即数，并将其扩展为32位
        }
    }
    
    when(io.outToRF.r1REn === true.B) { // 当寄存器1读使能有效时，将操作数1重定向为寄存器1读数据
        io.outToD.source1 := io.inFromRF.r1RData
    }.elsewhen(io.outToRF.r1REn === false.B) { // 当寄存器1读使能无效时，将操作数1重定向为立即数
        io.outToD.source1 := imm
    }
    
    when(io.outToRF.r2REn === true.B) { // 当寄存器2读使能有效时，将操作数2重定向为寄存器2读数据
...
object DC extends App {
    (new chisel3.stage.ChiselStage).execute(Array("--target-dir", "generated\\ID"), Seq(ChiselGeneratorAnnotation(() => new DC)))
}
